...
check("whole Edit writes ciphertext the same password opens", function()
  ...
  end, "a successful whole-file Edit save must dispose its plaintext scratch")
  assert_eq(vim.api.nvim_get_current_buf(), source, "whole-file Edit should return to its source buffer")
  assert_true(not vim.bo[source].modified, "the refreshed ciphertext source should be clean")
  assert_true((lines(source)[1] or ""):match("^%$ANSIBLE_VAULT") ~= nil, "the source should show ciphertext")
...
check("Create writes a file that only ever held ciphertext", function()
  ...
  vim.api.nvim_buf_set_lines(scratch, 0, -1, false, { "brand: new" })
  vim.cmd("silent write")
  wait_until(function()
    return not vim.api.nvim_buf_is_valid(scratch)
  end, "a successful Create save must dispose its plaintext scratch")
  assert_eq(vim.api.nvim_buf_get_name(0), path, "Create should open the ciphertext target after saving")
  assert_eq(vim.bo[0].buftype, "")
...
check("inline Edit writes the edited value back into the buffer only", function()
  ...
  wait_until(function()
    return not vim.api.nvim_buf_is_valid(scratch)
  end, "a successful inline Edit save must dispose its plaintext scratch")
  assert_eq(vim.api.nvim_get_current_buf(), source, "inline Edit should return to its source buffer")
