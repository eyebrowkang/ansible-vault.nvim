...
任何版本都可能变更或移除命令、配置、Lua API、事件及行为，不保证提供弃用过渡期、
兼容别名、迁移工具或迁移指南。
...
`:VaultEdit` 在单独的 scratch buffer 里编辑，不动原 buffer。
...
测试工作流与提交信息规范见 [CONTRIBUTING.md](CONTRIBUTING.md)，release notes 根据提交记录生成。
